...
class AudioSaver:
    ...
    def _save_mp3(
        ...
        try:
            import soundfile as _sf_mp3
            _audio_np = tensor_to_save.transpose(0, 1).numpy()  # [channels, samples] -> [samples, channels]
            _sf_mp3.write(str(temp_wav_path), _audio_np, int(target_sample_rate), subtype='PCM_16', format='WAV')
            cmd = [
