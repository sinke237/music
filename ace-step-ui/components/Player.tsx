...
export const Player: React.FC<PlayerProps> = ({
    ...
    // Karaoke: parse LRC and compute active lyric line
    const lrcLines = useMemo(() => {
        const lrcText: string = (currentSong?.generationParams as any)?.lrcText || '';
        if (!lrcText) return [];
        const re = /\[(\d+):(\d+(?:\.\d+)?)\]\s*(.*)/;
        return lrcText.split('\n').reduce<{ time: number; text: string }[]>((acc, line) => {
            const m = re.exec(line);
            if (m && m[3].trim()) {
                acc.push({ time: Number.parseInt(m[1]) * 60 + Number.parseFloat(m[2]), text: m[3].trim() });
            }
            return acc;
        }, []);
    }, [currentSong?.id]);
        ...
        if (isFullscreen) {
            return (
                <div className="fixed inset-0 z-50 bg-gradient-to-b from-zinc-100 to-zinc-50 dark:from-zinc-900 dark:to-black flex flex-col safe-area-inset-top safe-area-inset-bottom transition-colors duration-300">
                    ...
                    {/* Karaoke lyrics */}
                    {lrcLines.length > 0 && (
                        <div className="px-6 mb-3 min-h-[28px] flex items-center justify-center">
                            <p className="text-center text-base font-semibold text-zinc-800 dark:text-white transition-all duration-300 truncate">
                                {activeLyric}
                            </p>
                        </div>
                    )}
                    ...
                    {showDropdown && (
                        <div className="absolute bottom-24 left-1/2 -translate-x-1/2">
                            <SongDropdownMenu
                                ...
                                onCreateWanVideo={onOpenWanVideo}
                        ...
                        <div className="flex flex-col items-center lg:items-start gap-6 flex-1 min-w-0 max-w-lg">
                            ...
                            {lrcLines.length > 0 && (
                                <div className="min-h-[32px] flex items-center justify-center">
                                    <p className="text-center text-lg font-semibold text-zinc-800 dark:text-white transition-all duration-300 truncate max-w-lg">
                                        {activeLyric}
                                    </p>
                                </div>
                            )}
