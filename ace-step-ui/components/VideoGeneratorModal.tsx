...
export const VideoGeneratorModal: React.FC<VideoGeneratorModalProps> = ({ isOpen, onClose, song, mode = 'visualizer' }) => {
  ...
  // Wan integration state
  const [wanPrompt, setWanPrompt] = useState('');
  const [wanImageFile, setWanImageFile] = useState<File | null>(null);
  const [wanJobId, setWanJobId] = useState<string | null>(null);
  const [wanStatus, setWanStatus] = useState<string | null>(null);
  const [wanResultUrls, setWanResultUrls] = useState<string[]>([]);
  ...
  const [lrcAvailable, setLrcAvailable] = useState(false);
  const lrcLinesRef = useRef<{ time: number; text: string }[]>([]);
  const karaokeEnabledRef = useRef(karaokeEnabled);
  useEffect(() => { karaokeEnabledRef.current = karaokeEnabled; }, [karaokeEnabled]);

  // Parse LRC text into timed entries on song change
  useEffect(() => {
    const lrcText: string = (song?.generationParams as any)?.lrcText || '';
    if (!lrcText) { lrcLinesRef.current = []; setLrcAvailable(false); return; }
    const re = /\[(\d+):(\d+(?:\.\d+)?)\]\s*(.*)/;
    lrcLinesRef.current = lrcText.split('\n').reduce<{ time: number; text: string }[]>((acc, line) => {
      const m = re.exec(line);
      if (m && m[3].trim()) {
        acc.push({ time: parseInt(m[1]) * 60 + parseFloat(m[2]), text: m[3].trim() });
      }
      return acc;
    }, []);
    setLrcAvailable(lrcLinesRef.current.length > 0);
  }, [song]);
  ...
  useEffect(() => {
    if (song) {
        ...
        // Prefill Wan prompt with song title by default
        setWanPrompt(`${song.title} — cinematic music video`);
    }
  }, [song]);
      ...
      poll = setInterval(async () => {
        try {
          ...
          if (data.status === 'succeeded') {
            setWanResultUrls(data.result?.files || data.result?.urls || []);
          ...
          console.error('Wan status poll error', err);
        }
      }, 4000);
    }
  ...
  const renderOffline = async () => {
    ...
    for (let frameIndex = 0; frameIndex < totalFrames; frameIndex++) {
      ...
      // Karaoke lyrics overlay (use frame time for offline render)
      drawKaraokeLyric(ctx, width, height, time);
  ...
  const renderLoop = () => {
    ...
    drawKaraokeLyric(ctx, width, height, audioRef.current?.currentTime ?? 0);
  ...
  const drawKaraokeLyric = (ctx: CanvasRenderingContext2D, width: number, height: number, currentTime: number) => {
    const lines = lrcLinesRef.current;
    if (!lines.length || !karaokeEnabledRef.current) return;
    let lyric = '';
    for (const line of lines) {
      if (line.time <= currentTime) lyric = line.text;
      else break;
    }
    if (!lyric) return;
    ...
    const metrics = ctx.measureText(lyric);
    const padX = fontSize * 0.6;
    const padY = fontSize * 0.4;
    const boxW = metrics.width + padX * 2;
    const boxH = fontSize + padY * 2;
    const boxX = width / 2 - boxW / 2;
    const boxY = height * 0.74 - boxH / 2;
    const r = boxH / 2;
    ...
    ctx.fillText(lyric, width / 2, boxY + padY + fontSize * 0.85);
    ctx.restore();
  };
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/90 backdrop-blur-md p-0 md:p-4 animate-in fade-in duration-200">
      ...
      <div className={`bg-suno-card w-full h-full md:max-w-7xl md:h-[90vh] md:rounded-2xl border-0 md:border border-white/10 overflow-hidden shadow-2xl relative ${isMobile ? 'flex flex-col' : 'flex'}`}>
        ...
        <div className={`${isMobile ? 'flex-1 overflow-hidden' : 'w-96'} bg-suno-panel ${isMobile ? '' : 'border-r border-white/5'} flex flex-col z-20`}>
            ...
            {/* Karaoke Lyrics Toggle — always visible above tab content */}
            <div className="px-4 py-2 border-b border-white/5">
                <label className={`flex items-center gap-2 text-sm cursor-pointer select-none ${lrcAvailable ? 'text-zinc-300 hover:text-white' : 'text-zinc-600 cursor-not-allowed'}`}>
                    <input
                        type="checkbox"
                        checked={karaokeEnabled}
                        onChange={e => setKaraokeEnabled(e.target.checked)}
                        disabled={!lrcAvailable}
                        className="accent-pink-500 w-4 h-4 disabled:opacity-40"
                    />
                    <span>Karaoke lyrics overlay</span>
                    {!lrcAvailable && (
                        <span className="text-xs text-zinc-600 ml-auto">No lyrics — regenerate with LRC enabled</span>
                    )}
                </label>
            </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-6 space-y-4 md:space-y-6">
                ...
                {/* WAN Mode: Prompt + Image + Generate */}
                {mode === 'wan' && (
                  <div className="bg-black/10 p-3 rounded-lg border border-white/5 space-y-3">
                    <h3 className="text-sm font-bold">Create video with Wan</h3>
                    <p className="text-[11px] text-zinc-400">Supply a prompt and optional image. The selected audio will be sent to Wan for cinematic video generation.</p>
                    <textarea
                      value={wanPrompt}
                      onChange={(e) => setWanPrompt(e.target.value)}
                      placeholder="Describe the cinematic scene, mood, camera, and visuals..."
                      className="w-full min-h-[80px] bg-zinc-900 rounded p-2 text-xs text-white border border-white/5"
                    />
                    <div className="flex items-center gap-2">
                      <input type="file" accept="image/*" onChange={(e) => setWanImageFile(e.target.files ? e.target.files[0] : null)} />
                      <button
                        onClick={async () => {
                          if (!song) return;
                          setWanGenerating(true);
                        ...
                        disabled={wanGenerating || !wanPrompt || !song}
                        className="py-2 px-3 bg-pink-600 rounded text-white text-sm disabled:opacity-50"
                      >
                        {wanGenerating ? 'Generating...' : 'Generate with Wan'}
                      </button>
                    </div>

                    <div className="text-[12px] text-zinc-400">
                      <div>Status: {wanStatus || 'idle'}</div>
                      {wanJobId && <div>Job: {wanJobId}</div>}
                      {wanResultUrls.length > 0 && (
                        <div className="space-y-2 mt-2">
                          {wanResultUrls.map((url, idx) => (
                            <div key={idx} className="bg-black/20 p-2 rounded">
                              <video src={url} controls className="w-full h-auto rounded" />
                              <a className="text-[12px] text-emerald-400" href={url} target="_blank" rel="noreferrer">Download</a>
                            </div>
                          ))}
                        </div>
                      )}
