...
async function buildGradioArgs(params: GenerationParams): Promise<unknown[]> {
  ...
  return [
    ...
    referenceAudio,                                               // 10: reference_audio (filepath | null)
    params.duration && params.duration > 0 ? params.duration : -1, // 11: audio_duration (-1 = auto)
    Math.min(Math.max(params.batchSize ?? 1, 1), 16),            // 12: batch_size_input (clamped 1-16)
    sourceAudio,                                                  // 13: src_audio (filepath | null)
    ...
    0.0,                                                          // 19: cover_noise_strength (default 0.0)
    (params.taskType === 'audio2audio' ? 'cover' : params.taskType) || 'text2music', // 20: task_type
    false,                                                        // 21: no_fsq (ACE-Step v1.5 new param, default false)
    ...
    'euler',                                                      // 27: sampler_mode (ACE-Step v1.5, default 'euler')
    0.0,                                                          // 28: velocity_norm_threshold (ACE-Step v1.5, 0 = disabled)
    0.0,                                                          // 29: velocity_ema_factor (ACE-Step v1.5, 0 = disabled)
    true,                                                         // 30: dcw_enabled (ACE-Step v1.5, default true)
    'double',                                                     // 31: dcw_mode (ACE-Step v1.5, default 'double')
    0.05,                                                         // 32: dcw_scaler (ACE-Step v1.5, default 0.05)
    0.02,                                                         // 33: dcw_high_scaler (ACE-Step v1.5, default 0.02)
    'haar',                                                       // 34: dcw_wavelet (ACE-Step v1.5, default 'haar')
    params.customTimesteps || '',                                 // 35: custom_timesteps
    params.audioFormat || 'mp3',                                  // 36: audio_format
    '128k',                                                       // 37: mp3_bitrate (ACE-Step v1.5, default '128k')
    48000,                                                        // 38: mp3_sample_rate (ACE-Step v1.5, default 48000)
    params.lmTemperature ?? 0.85,                                 // 39: lm_temperature
    isThinking,                                                   // 40: think_checkbox
    params.lmCfgScale ?? 2.0,                                    // 41: lm_cfg_scale
    params.lmTopK ?? 0,                                           // 42: lm_top_k
    params.lmTopP ?? 0.9,                                         // 43: lm_top_p
    ...
    // NOTE: wiring pos 48 is is_format_caption_state (gr.State) — the Gradio client
    // automatically inserts null at all State positions before sending; do NOT include
    // it here or all subsequent args shift by one and score_scale receives False.
    params.constrainedDecodingDebug ?? false,                     // 48→49: constrained_decoding_debug
    params.allowLmBatch ?? true,                                  // 49→50: allow_lm_batch
    params.getScores ?? false,                                    // 50→51: auto_score
    params.getLrc ?? false,                                       // 51→52: auto_lrc
    params.scoreScale ?? 0.5,                                     // 52→53: score_scale (min 0.01)
    params.lmBatchChunkSize ?? 8,                                 // 53→54: lm_batch_chunk_size
    params.trackName || null,                                     // 54→55: track_name
    params.completeTrackClasses || [],                            // 55→56: complete_track_classes
    true,                                                         // 56→57: enable_normalization (default true)
    -1.0,                                                         // 57→58: normalization_db (default -1.0)
    0.0,                                                          // 58→59: fade_in_duration (ACE-Step v1.5, default 0.0)
    0.0,                                                          // 59→60: fade_out_duration (ACE-Step v1.5, default 0.0)
    0.0,                                                          // 60→61: latent_shift (default 0.0)
    1.0,                                                          // 61→62: latent_rescale (default 1.0)
    'balanced',                                                   // 62→63: repaint_mode (ACE-Step v1.5, default 'balanced')
    0.5,                                                          // 63→64: repaint_strength (ACE-Step v1.5, default 0.5)
    params.autogen ?? false,                                      // 64→65: autogen_checkbox
    // Note: is_format_caption_state (pos 48), current_batch_index, total_batches,
    // batch_queue, generation_params_state (pos 66-69) are gr.State variables.
    // The Gradio client inserts null placeholders at those positions automatically.
  ];
}
...
async function processGenerationViaGradio(
  ...
  // Extract LRC lyric timestamps from outputs 36-43 (lrc_display_1..8)
  const lrcTexts: string[] = [];
  for (let i = 0; i < 8; i++) {
    const lrc = data[36 + i];
    lrcTexts.push(typeof lrc === 'string' ? lrc.trim() : '');
  }
